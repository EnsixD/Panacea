...
Item {
    ...
    Timer {
        ...
        onTriggered: { pAgents.running = false; pAgents.running = true; }
    }
        ...
        Repeater {
            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    // Лимитов нет — так и говорим. Пустая полоска на нуле
                    // читалась бы как «ничего не израсходовано», а это неправда:
                    // мы просто не знаем.
                    Text {
                        Layout.fillWidth: true
                        visible: (card.modelData.limits || []).length === 0
                        text: view.sys.tr("Лимиты этот агент наружу не отдаёт")
                        color: view.sys.colMuted
