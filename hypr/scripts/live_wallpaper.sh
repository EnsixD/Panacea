...
set)
    ...
    # * -p и -a MAX: mpv засыпает, когда обои не видно — под полноэкранным
    #   окном или развёрнутым на весь экран. Иначе видео крутится вхолостую
    #   и греет ноутбук.
    # * loop-inf и no-audio: обои не должны кончаться и не должны звучать.
    # * hwdec=auto-safe: декодирование уезжает в видеокарту, где это возможно.
    mpvpaper -f -p -a MAX -l background -o \
        "--loop-inf --no-audio --hwdec=auto-safe --vo=gpu --profile=low-latency" \
        '*' "$vid" >/dev/null 2>&1
